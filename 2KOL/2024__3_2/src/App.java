...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(najdiTezina(drvo.root));

    }
    

    private static int najdiTezina(BNode<Integer> root){
        if(root == null){
            return 0;
        }
        if(root.left != null && root.right != null){
            return root.info + Math.max(najdiTezina(root.left) , najdiTezina(root.right));
        }else if (root.left != null && root.right == null) {
            return root.info + najdiTezina(root.left);
        }else if (root.left == null && root.right != null) {
            return root.info + najdiTezina(root.right);
        }else{
            return root.info;
        }
    }
